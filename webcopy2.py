...
#         return sorted([(k, float(rankedURLSDict[k] - min_url_rank)
    ...
    def ratio_et(self):
        external_links = self.external
        summ = self.summ
        c=[x / y if y else 0 for x, y in zip(external_links, summ)]  
        #print(str(c)) # Test
        return c 
    ...
    def feature_extract(self):
        ...
        ext,int = self.ext_links()
        #print(ext) test for external links
        #print(int) test for internal links
        we = self.ratio_we()
        et = self.ratio_et()
